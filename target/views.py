...
def target_list(request):
	...
	if records_total > 0:
		...
		for record in result:
			...
			json_record['target_cpu'] = '<span id="span_id_'+str(i)+'_2"  class=\'green_values\'>'+str(record["target_cpu"])+'</span><input type="hidden" name="target_id_'+str(i)+'_2" id="target_id_'+str(i)+'_2" value="'+str(record["id"])+'"><input type="hidden" name="target_cpu_'+str(i)+'_2" id="target_cpu_'+str(i)+'_2" value="'+str(record["target_cpu"])+'"><input type="hidden" name="target_cpu_field_'+str(i)+'_2" id="target_cpu_field_'+str(i)+'_2" value="'+str('target_cpu')+'">'
			# json_record['no_of_temp_labour'] = '<input type="text" name="temp_labor_'+str(i)+'_1" id="temp_labor_'+str(i)+'_1" value="'+str(record.id)+'">'
			json_record['target_cpg'] ='<span id="span_id_'+str(i)+'_3" class=\'green_values\'>'+str(record["target_cpg"])+'</span><input type="hidden" name="target_id_'+str(i)+'_3" id="target_id_'+str(i)+'_3" value="'+str(record["id"])+'"><input type="hidden" name="target_cpg_'+str(i)+'_3" id="target_cpg_'+str(i)+'_3" value="'+str(record["target_cpg"])+'"><input type="hidden" name="target_cpg_field_'+str(i)+'_3" id="target_cpg_field_'+str(i)+'_3" value="'+str('target_cpg')+'">'
...
def target_form(request):
	...
	if request.method == "POST":
		...
		# target_ids = request.POST.get('id','')
		# target_id = str(target_ids)
		# print(target_id)

		# b1 = GdbTarget.objects.filter(target_projectname=target_projectname).values_list('id', flat=True);
		# print(b1)
		# if b1 == '':
		...
		#To save the master_projects table project id in Target table column target_project_id_id
...
@csrf_exempt
def delete_project_target(request):
  if request.method == "POST":
    
    target_id=request.POST['deleteids_arr']
    selected_leads=json.loads(target_id)

    print(selected_leads)
   
    for i,del_master_id in enumerate(selected_leads):      
      del_target = GdbTarget.objects.filter(id=del_master_id).delete()
      if del_target:
