...
def import_data(request):
    ...
    if request.method == "POST":
      ...
      data[['TEMP_LABOR_COST','TOTAL_UNITS','UNITS_G1','UNITS_G5']] = data[['TEMP_LABOR_COST','TOTAL_UNITS','UNITS_G1','UNITS_G5']].apply(pd.to_numeric, errors='ignore')
      print(data[['TOTAL_UNITS','UNITS_G1','UNITS_G5']])
      
      
      data[['TEMP_LABOR_COST']] = data[['TEMP_LABOR_COST']].astype(float,errors="ignore")
      data[['TOTAL_UNITS','UNITS_G1','UNITS_G5']] = data[['TOTAL_UNITS','UNITS_G1','UNITS_G5']].fillna(0.0).astype(int,errors="ignore")
      ...
      type_and_error = {'date':['DateField','Date field error'],'week':['week','character field error'],'month':['month','character field error'],'project group':['project group','character field error'],'project_name':['project_name','character field error'],'shift':['shift','character field error'],'q#':['q#','character field error'],'customer':['customer','character field error'],'whse #':['int','Values must be in integer round values'],'supervisor_name':['supervisor_name','character field error'],'work_type':['work_type','character field error'],'product_category':['product_category','character field error'],'size':['size','character field error'],'temp_labor_hrs':['int','Values must be in integer round values'],'gdb labor hrs':['int','Values must be in integer round values'],'temp_labor_cost':['float','Values must be in decimal'],'gdb_labor_cost':['float','Values must be in decimal'],'total_labor_cost':['float','Values must be in decimal'],'units_g1':['int','Values must be in integer round values'],'units_g5':['int','Values must be in integer round values'],'units_q':['int','Values must be in integer round values'],'units_p':['int','Values must be in integer round values'],'units_other':['int','Values must be in integer round values'],'total_units':['int','Values must be in integer round values'],'total_gallons_done':['float','Values must be in decimal'],'cost_gallon':['float','Values must be in decimal'],'cost_unit':['float','Values must be in decimal'],'description_comments':['description_comments','character field error'],'chris lampe comment/notes':['chris lampe comment/notes','character field error'],"laurie's/intern's comments":["laurie's/intern's comments",'character field error']}
      ...
      with open(filename, 'w+') as file:
          ...
          for i,j in data.iterrows():
            ...
            for k,v in enumerate(header):
              ...
              # reason={[]}
              if str(j[k]).strip() =="" or str(j[k]).strip() =="$-":
                j[k] =""
                continue
