...
def edit_list_home(request):
  ...
  if request.method == "POST":
    ...
    for master_relation in total_related_master:
          ...
          try:
            master_json['project_tmp_labor_cost'] =float(master_relation['project_tmp_labor_hrs']) * 14.50
          except Exception:
            master_json['project_tmp_labor_cost'] = 0

          try:
            master_json['project_gdb_labor_cost'] =float(master_relation['project_gdb_labor_hrs']) * 11.00
          except Exception:
            master_json['project_gdb_labor_cost'] = 0
          ...
          master_json['project_total_labor_cost'] = master_json['project_tmp_labor_cost'] + master_json['project_gdb_labor_cost']
          ...
          print(float(master_json['unit_done_count']),master_json['product_category'],float(master_json['project_total_labor_cost']),"DONE CHECK")
          master_json['gallon_done_count'] = master_json['master_units_total_gallon_cal'] + master_json['master_qp_total_gallon_cal']
          
          try:
            master_json['master_cost_unit_count'] = float(master_json['project_total_labor_cost']) / float(master_json['unit_done_count'])
          except Exception:
            master_json['master_cost_unit_count'] = 0
          try:
            master_json['master_cost_gallon_count'] = float(master_json['project_total_labor_cost']) / float(master_json['gallon_done_count'])
          except Exception:
            master_json['master_cost_gallon_count'] = 0
          # master_json['master_cost_unit_count'] = float(master_relation['project_total_labor_cost']) / float(master_json['unit_done_count']) if float(master_json['unit_done_count']) is not None and float(master_json['unit_done_count'])!=0 else 0
...
def edit_master(request):
    ...
    if clicked_field == "project_tmp_labor_hrs" or clicked_field=="project_gdb_labor_hrs":
        ...
        if clicked_field=="project_tmp_labor_hrs":
           ...
           tmp_labor_cost_update_value =float(clicked_value) *  14.50
        ...
        if clicked_field=="project_gdb_labor_hrs":
           ...
           gdb_labor_cost_update_value =float(clicked_value) *  11.00
...
def summary(request):
    ...
    total_related_labor=GdbLabour.objects.filter(date=today_date).values('id','date','labour_cost','no_of_temp_labour','no_of_gdb_labour')
    total_related_master=Master.objects.filter(project_date=today_date).annotate(master_rel_count=Count('relationship__master_id'),  
...
def report(request):
  if request.method == "POST":
    ...
    if request.POST['min'] =="":
       ...
       today = date.today() 
       today_date = today.strftime("%Y-%m-%d") 
       Labor_qry=GdbLabour.objects.filter(date=today_date)
    else:
       # print(type(request.POST['min']))
       # print("tWO")
...
def edit_list_summary(request):
   if request.method == "POST":
    ...
    if request.POST['min'] =="":
       ...
       today = date.today() 
       today_date = today.strftime("%Y-%m-%d") 
       Project_qry=Project.objects.filter(master__project_date=today_date)
...
def savings_loss(request):
    ...
    if project_max_date['project_date__max'] != None or project_max_date['project_date__max']=='':
      max_date=project_max_date['project_date__max'].strftime("%B %d, %Y")
    else:
      max_date =''
    ...
    "project_max_date":max_date, "project_min_date":project_min_date
    }
...
def savings_loss_list(request):
      ...
      today = datetime.datetime.now()
      total_related_project=Project.objects.filter(master__project_date__year=today.year).annotate(master_total_labor_cost=Sum('master__project_total_labor_cost'),
        master_unit_done_count=Sum('master__quartpint__project_total_units'),
        master_gallon_done_count=Sum('master__quartpint__project_total_gallons'),
        master_cost_per_gallon=Sum('master__quartpint__project_cost_gallon'),
        master_cost_per_unit=Sum('master__quartpint__project_cost_unit'),
        ...
        dcount_master=Count('master'),
        others=Count('master__quartpint__others'),
        # eighteen_year_target=Sum('master__eighteen_year_target'),
        ...
        'master_total_labor_cost',
        'master_unit_done_count','master_gallon_done_count','master_cost_per_gallon','master_cost_per_unit',
        'others','master_cost_per_gallon_count','master_cost_per_unit_count','project_ids__target','dcount_master','eighteen_year_target')
      # print(total_related_project.query)
      ...
      for master_relation in total_related_project:
            ...
            if master_relation['master_total_labor_cost'] != 0:
              master_json['id'] = master_relation['dcount_master']
              master_json['project_id'] = master_relation['id']
              ...
              # else:
              # master_json['last_year']=master_relation['eighteen_year_target']
              # master_json['last_year']='$'
              master_json['2108_year']=master_relation['eighteen_year_target'] if master_relation['eighteen_year_target']!=None else 0.00

              master_json['last_year']='<span id="span_id_'+str(i)+'_1"  class=\'green_values\'>$'+str(master_json['2108_year'])+'</span><input type="hidden" name="master_id_'+str(i)+'_1" id="master_id_'+str(i)+'_1" value="'+str(master_json['project_id'])+'"><input type="hidden" name="last_year_'+str(i)+'_1" id="last_year_'+str(i)+'_1" value="'+str(master_json['2108_year'])+'"><input type="hidden" name="last_year_field_'+str(i)+'_1" id="last_year_field_'+str(i)+'_1" value="'+str('last_year')+'">'
              if master_relation['others']=='':
                 master_json['minus_pl_gallon']=round(float(master_json['summary_target']),2)-round(float(master_json['master_cost_per_gallon']),2)
                 master_json['multi_pl_gallon']=round(float(master_json['minus_pl_gallon']),2) * round(float(master_json['gallon_done_count']),2)
                 master_json['pl_color']=round(float(master_json['multi_pl_gallon']),2)
...
def graph_representation(request):
    ...
    for i in range(1,8):
      result={}
      sdate = datetime.datetime.today() - datetime.timedelta(days=i)
      start_date = sdate.strftime("%Y-%m-%d")
      try:
        res = GdbLabour.objects.get(date=start_date)
      except GdbLabour.DoesNotExist:
        res = None
      # print(res.temp_cost)
      if res:
        result['las_date'] =sdate
        result['temp_cost'] =res.temp_cost
        result['gdb_cost'] = res.gdb_cost
      else:
        result['las_date'] =sdate
        result['temp_cost'] = 0
        result['gdb_cost'] = 0 
      labour_cost.append(result)
    # labour_cost   = GdbLabour.objects.filter(date__year=today.year).all().order_by('-date')[:7] 
...
def get_project_profilt_loss(request,param=None):
    if param != 'chart':
      ...
      today = datetime.datetime.now()
      Project_qry=Project.objects.filter(master__project_date__year=today.year)

    # total_related_project=Project_qry.filter(master__project_total_labor_cost__isnull=False).select_related('master__project_id').select_related('master__quartpint__master_id').annotate(master_total_labor_cost=Sum('master__project_total_labor_cost'),
    #     master_unit_done_count=Sum('master__quartpint__project_total_units'),
    #     master_gallon_done_count=Sum('master__quartpint__project_total_gallons'),
    #     master_cost_per_gallon=Sum('master__quartpint__project_cost_gallon'),
    #     master_cost_per_unit=Sum('master__quartpint__project_cost_unit'),
    ...
    total_related_project=Project_qry.annotate(master_total_labor_cost=Sum('master__project_total_labor_cost'),
        master_unit_done_count=Sum('master__quartpint__project_total_units'),
        master_gallon_done_count=Sum('master__quartpint__project_total_gallons'),
        master_cost_per_gallon=Sum('master__quartpint__project_cost_gallon'),
        master_cost_per_unit=Sum('master__quartpint__project_cost_unit'),
...
def import_data(request):
    ...
    if request.method == "POST":
      ...
      data = pd.read_csv(request.FILES['data'],skipinitialspace=True,dtype='object',keep_default_na=False,error_bad_lines=False,warn_bad_lines=False)

      # data.head()
      data.columns = data.columns.str.strip()
      header = list(data.columns)
      # data[['WHSE #','TEMP_LABOR_HRS','TEMP_LABOR_COST','GDB LABOR HRS','TOTAL_UNITS','UNITS_G1','UNITS_G5']] = data[['WHSE #','TEMP_LABOR_HRS','TEMP_LABOR_COST','GDB LABOR HRS','TOTAL_UNITS','UNITS_G1','UNITS_G5']].apply(pd.to_numeric, errors='ignore')
      
      # data[['TEMP_LABOR_COST']] = data[['TEMP_LABOR_COST']].astype(float,errors="ignore")
      # data[['WHSE #','TEMP_LABOR_HRS','GDB LABOR HRS','TOTAL_UNITS','UNITS_G1','UNITS_G5']] = data[['WHSE #','TEMP_LABOR_HRS','GDB LABOR HRS','TOTAL_UNITS','UNITS_G1','UNITS_G5']].fillna(0).astype(int,errors="ignore")
      ...
      # next(io_string)
      header = [x.strip().lower() for x in header if x]

      data.columns = header

      matched_alldate=Master.objects.all().values('project_date','project_id_id__project_name','project_shift')
      data_set = {}
      for x in matched_alldate:
        ...
        if x['project_date'] not in data_set:
          data_set[x['project_date']] = [x['project_id_id__project_name'],x['project_shift']]
        else:
          if x['project_id_id__project_name'] not in data_set[x['project_date']]:
            data_set[x['project_date']].append(x['project_id_id__project_name'])

          if x['project_shift'] not in data_set[x['project_date']]:
            data_set[x['project_date']].append(x['project_shift'])
      
      data_list = ["DATE","WEEK","MONTH","PROJECT GROUP","PROJECT_NAME","SHIFT","Q#","CUSTOMER","WHSE #","SUPERVISOR_NAME","WORK_TYPE","PRODUCT_CATEGORY","SIZE","TEMP_LABOR_HRS","GDB LABOR HRS","TEMP_LABOR_COST","GDB_LABOR_COST","TOTAL_LABOR_COST","UNITS_G1","UNITS_G5","UNITS_Q","UNITS_P","UNITS_OTHER","TOTAL_UNITS","TOTAL_GALLONS_DONE","COST_GALLON","COST_UNIT","DESCRIPTION_COMMENTS","CHRIS LAMPE COMMENT/NOTES","LAURIE'S/INTERN'S COMMENTS"]
      
      data_list = [x.strip().lower() for x in data_list]
      type_and_error = {
                        'date':['DateField','Should be in Date Format - MM/DD/YYYY'],
                        # 'whse #':['int','Values must be in integer round values'],
                        'temp_labor_hrs':['intfloat','Values must be in decimal'],
                        'gdb labor hrs':['intfloat','Values must be in decimal'],
                        'temp_labor_cost':['float','Values must be in decimal'],
                        'gdb_labor_cost':['float','Values must be in decimal'],
                        'total_labor_cost':['float','Values must be in decimal'],
                        'units_g1':['int','Values must be in integer round values'],
                        'units_g5':['int','Values must be in integer round values'],
                        'units_q':['int','Values must be in integer round values'],
                        'units_p':['int','Values must be in integer round values'],
                        'units_other':['int','Values must be in integer round values'],
                        'total_units':['int','Values must be in integer round values'],
                        'total_gallons_done':['intfloat','Values should be numeric'],
                        'cost_gallon':['float','Values must be in decimal'],
                        'cost_unit':['float','Values must be in decimal'],
                        'project_name':['char','Values should not be empty'],
                        'shift':['char','Values should not be empty']
                        }
      
      type_and_error_keys = list(type_and_error.keys())


      if not set(data_list).issubset(header):
        # print("COMPARE TEST")
        status="header"
      ...
      # ct=0
      today = datetime.datetime.now()
      file_name  = 'Master_Invalid_Records('+today.strftime("%d_%m_%Y_%H_%M_%S")+').csv'        
      filename = settings.MEDIA_ROOT+'/master/'+file_name
      ...
      with open(filename, 'w+') as file:
          ...
          for i,j in data.iterrows():
            ...
            for k,v in enumerate(header):
              ...
              if v in type_and_error_keys:


                datatype = type_and_error[v][0]
                # reason={[]}
                if (str(j[k]).strip() =="" or str(j[k]).strip() =="$-") and datatype!='char':
                  j[k] =""
                  # reason+= v+" -> "+'value should not be empty' +" -> "+ str(j[k]) + "\n"
                  # data_break = True
                  continue
                ...
                if datatype=="DateField":
                  ...
                  try:
                    # datetime.datetime.
                    datetime.datetime.strptime(j[k], '%m/%d/%Y').strftime("%Y-%m-%d")

                  except:
                    reason+= v+" -> "+type_and_error[v][1] +" -> "+ j[k] + "\n"
                    data_break = True
                ...
                if datatype=="int":
                  ...
                  try:
                    ...
                    check = int(str(j[k]).strip("$"))
                   
                  except Exception as e:
                    print("qwe")
                    print(e)
                    reason+= v+" -> "+type_and_error[v][1] +" -> "+ str(j[k]) + "\n"
                    data_break = True
                ...
                if datatype=="float":
                  ...
                  try:
                    str(j[k]).split('.')[1]
                    check = float(str(j[k]).strip().strip("$").replace(',',''))
                ...
                if datatype=="char":
                  try:
                    if not str(j[k]).strip():
                      raise ValueError('error')
                  except Exception as e:
                    reason+= v+" -> "+type_and_error[v][1] +" -> "+ str(j[k]) + "\n"
                    data_break = True

                if datatype=="intfloat":
            ...
            if not str(j[0]).strip() or  not str(j[4]).strip() or not str(j[5]).strip():
              writer.writerow(list(j)+[reason])
              continue

            # data_insert=True

            # if not j[0] and  not j[4] and not j[5]:
            #   continue

            master_date= datetime.datetime.strptime(j[0], '%m/%d/%Y').strftime("%Y-%m-%d") if j[0] else ""
            
            # print(j[4],j[5],master_date)
            # print(data_set)

            if data_set.get(master_date,False):
              status =''
              if j[4] in data_set.get(master_date) and j[5] in data_set.get(master_date):
                reason="(Projectname,Projectdate,Shift) may be repeated"
                writer.writerow(list(j)+[reason])
            ...
            if data_insert:
              ...
              if master_date not in data_set:
                data_set[master_date] = [j[4],j[5]]
              else:
                if j[4] not in data_set[master_date]:
                  data_set[master_date].append(j[4])

                if j[5] not in data_set[master_date]:
                  data_set[master_date].append(j[5])
              ...
              project_temp_labor_cost_split=str(project_temp_labor_cost).split("$")
              project_gdb_labor_cost_split=str(project_gdb_labor_cost).split("$")
              project_total_labor_cost_split=str(project_total_labor_cost).split("$")
              ...
              try:
                  if cost_per_gallons_sht!='' or cost_per_gallons_sht!=0:
                     cost_per_gallons_split=str(cost_per_gallons_sht).split("$")
              except:
                  cost_per_gallons_split=0
              
              cost_per_units_split=str(cost_per_units_sht).split("$")
              ...
              try:
                 if cost_per_units_split[1]!='' or cost_per_units_split[1]!='-   ' or cost_per_units_split[1]!=0:
                    cost_per_units=float(cost_per_units_split[1].replace(',',''))
              except:
                  cost_per_units=0
    ...
    if fl_ct == 0:
        file_path = settings.MEDIA_ROOT+'/master/'+file_name                
        if os.path.exists(file_path):
            # print(file_name)        
            os.remove(file_path)             
    return JsonResponse({'status':status,'filename':file_name,'insert_rows':dl_ct,'duplicate_rows':fl_ct})
...
def view_details(request):
  if request.method == "POST":    
    ...
    today = datetime.datetime.now()
    total_related_master=Master_qry.filter(project_total_labor_cost__isnull=False).filter(project_date__year=today.year).annotate(master_rel_count=Count('relationship__master_id'),
       master_unit_count=Sum('relationship__project_units'),
       master_unit_done_count=Sum('quartpint__project_total_units'),
       master_qp_rel_count=Count('quartpint__master_id'),
...
def download(request,slug):    
    filename=slug
    try:    
        file_path = settings.MEDIA_ROOT+'/master/'+filename        
        if os.path.exists(file_path):
            with open(file_path, 'rb') as fh:
                response = HttpResponse(fh.read(), content_type="application/vnd.ms-csv")
...
@csrf_exempt
def edit_master_last_year(request):
    ...
    if clicked_field == "last_year":
     
     updat = {'eighteen_year_target': clicked_value}
     update_master=Project.objects.filter(id=m_id).update(**updat)
     working=updat
    return JsonResponse({'success':working})