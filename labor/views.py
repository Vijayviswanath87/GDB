...
def ajax_fetch_labour_list(request):
    ...
    if request.POST.get('min') and request.POST.get('max'):
      ...
      records      =GdbLabour.objects.filter(date__year=int(year),date__month=int(month)).all()
        ...
        # records_total = GdbLabour.objects.filter(date__year=2019,date__month=4).count()
        # records       = GdbLabour.objects.filter(date__year=2019,date__month=4).all()
        today = datetime.datetime.today() 
        records_total = GdbLabour.objects.filter(date__year=today.year,date__month=today.month).count()
        records = GdbLabour.objects.filter(date__year=today.year,date__month=today.month).all()
...
#         file_name  = 'Labor_Invalid_Records('+today.strftime("%d_%m_%Y_%H_%M_%S")+').csv'        
#         filename = settings.MEDIA_ROOT+'/labour/'+file_name
#         #  s
...
#                     labor_temp_cost=j[5] if j[5]!='' else ''
#                     labor_temp_cost_split=str(labor_temp_cost).split("$")
#                     try:
#                         if labor_temp_cost_split[1]!='':
#                              labor_temp_cost=float(labor_temp_cost_split[1].replace(',',''))
#                     except :
...
#                     labor_gdb_cost=j[6] if j[6]!='' else ''
#                     labor_gdb_cost_split=str(labor_gdb_cost).split("$")
#                     try:
#                         if labor_gdb_cost_split[1]!='':
#                              labor_gdb_cost=float(labor_gdb_cost_split[1].replace(',',''))
#                     except :
...
#                     labor_cost=j[7] if j[7]!='' else ''
#                     labor_cost_split=str(labor_cost).split("$")
#                     try:
#                         if labor_cost_split[1]!='':
#                              labor_cost=float(labor_cost_split[1].replace(',',''))
#                     except :
...
#                     try:
#                         if labor_profit_sht!='' or labor_profit_sht!=0:
#                            labor_profit_split=str(labor_profit_sht).split("$")
#                     except:
#                            labor_profit_split=0
...
#                     try:
#                         if labor_loss_sht!='' or labor_loss_sht!=0:
#                            labor_loss_split=str(labor_loss_sht).split("$")
#                     except:
#                            labor_loss_split=0
#                     # print(labor_loss_split[1],"SPLIT CHECK===>")     
#                     try:
...
#                     try:
#                         if labor_nl_sht!='' or labor_nl_sht!=0:
#                            labor_nl_split=str(labor_nl_sht).split("$")
#                     except:
#                            labor_nl_split=0
...
#                     # print("LABOR===>",labor_fields)
#                     try:
#                         result = GdbLabour.objects.create(**labor_fields)
#                         # if result:
#                         status ="success"
#                         # dl_ct= dl_ct + 1
#                     except:
...
#     if fl_ct == 0:
#         file_path = settings.MEDIA_ROOT+'/labour/'+file_name                
...
def labor_import_data(request):
    ...
    if request.method == "POST":
      # print(request.FILES['data'],"<==FILE==>")
        ...
        data_list = ["DATE","NO_OF_TEMP","NO_OF_GDB","TEMP_HOURS","GDB_HOURS","TEMP_COST","GDB_COST","LABOR_COST","PROFIT", "LOSS" , "NET_P_L"]
      
        data_list = [x.strip().lower() for x in data_list]
        type_and_error = {
                        'date':['DateField','Should be in Date Format - MM/DD/YYYY'],
                        'no_of_temp':['int','Values must be in integer round values'],
                        'no_of_gdb':['int','Values must be in integer round values'],
                        'temp_hours':['intfloat','Values must be in decimal'],
                        'gdb_hours':['intfloat','Values must be in decimal'],
                        'temp_cost':['float','Values must be in decimal'],
                        'gdb_cost':['float','Values must be in decimal'],
                        'labor_cost':['float','Values must be in decimal'],
                        'profit':['float','Values must be in decimal'],
                        'loss':['intfloat','Values must be in decimal'],
                        'net_p_l':['intfloat','Values must be in decimal']
        ...
        with open(filename, 'w+') as file:
            ...
            for i,j in data.iterrows():
                ...
                for k,v in enumerate(header):
                    if v in type_and_error_keys:
                        ...
                        if datatype=="intfloat":
                          ...
                          try:                    
                            # check = float(str(j[k]).strip().strip("$").replace(',',''))
                            check = float(str(j[k]).strip().strip("$").replace(',','').replace('(','').replace(')',''))
                          except Exception as e:
                            print(e)
                            reason+= v+" -> "+type_and_error[v][1] +" -> "+ str(j[k]) + "\n"
                            data_break = True
                ...
                if labor_date in data_set:
                    ...
                    try:
                        if labor_loss_sht!='' or labor_loss_sht!=0:
                          search_value = re.search(r"\$\((.*)\)",labor_loss_sht)
                          if search_value:
                            labor_loss_sht = '$-'+search_value.group(1)
                          labor_loss_split=str(labor_loss_sht).split("$")
                    except:
                           labor_loss_split=0
                    # print(labor_loss_split[1],"SPLIT CHECK===>")     
                    ...
                    try:
                        if labor_nl_sht!='' or labor_nl_sht!=0:
                          search_value = re.search(r"\$\((.*)\)",labor_nl_sht)
                          if search_value:
                              labor_nl_sht = '$-'+search_value.group(1)
                          labor_nl_split=str(labor_nl_sht).split("$")
                    except:
                           labor_nl_split=0
...
def download(request,slug):    
    filename=slug
    try:    
        file_path = settings.MEDIA_ROOT+'/labour/'+filename        
        if os.path.exists(file_path):
